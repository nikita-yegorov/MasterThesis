...
wandb.init(project="Discriminator S_T -> S_c__T_c", entity="nikita_yegorov")
...
for epoch in range(1, args.n_epoch + 1):
    ...
    for i, ((im_source, label_source), (im_target, label_target)) in enumerate(iters):
        ...
        if s_b !=CFG.batch_size or t_b != CFG.batch_size:
            continue
        im_source_ = im_source.to(DEVICE)
        im_target_ = im_target.to(DEVICE)

        # Train the discriminator  
        d_loss, source_score, target_score = train_discriminator(im_source_, im_target_)

        print('Epoch [{}/{}], Step [{}/{}], d_loss: {:.4f},  D(source): {:.2f}, D(target): {:.2f}' 
                  .format(epoch, args.n_epoch, i+1, iters, d_loss.item(),  
                          source_score.mean().item(), target_score.mean().item()))
        wandb.log({"d_loss_train": d_loss.item(),
                    "source_score_train": source_score.mean().item(),
                    "target_score_train": target_score.mean().item()
                    })
        if i % 10 == 0:
            with torch.no_grad():
                D.eval()
                c_im = im_source.detach().numpy()
                ...
                im_source_c = im_canny.to(DEVICE)

                c_im = im_target.detach().numpy()
                c_im = np.transpose(c_im,(0,2,3,1))
                c_im = np.mean(c_im, 3)
                edges1 = np.array([feat.canny(i) for i in c_im])
                edges1 = np.stack((edges1,)*3, axis=1)
                im_canny = torch.from_numpy(edges1)
                im_canny = im_canny.type(torch.float)
                im_target_c = im_canny.to(DEVICE)

                d_loss_c, source_score_c, target_score_c = train_discriminator(im_source_c, im_target_c, True)
                
                print('Epoch [{}/{}], Step [{}/{}], d_loss_canny: {:.4f},  D(source_canny): {:.2f}, D(target_canny): {:.2f}' 
                  .format(epoch, args.n_epoch, i+1, iters, d_loss_c.item(),  
                          source_score_c.mean().item(), target_score_c.mean().item()))

                wandb.log({"d_loss_train_canny": d_loss_c.item(),
                    "source_score_train_canny": source_score_c.mean().item(),
                    "target_score_train_canny": target_score_c.mean().item()
                    })          

                d_losses,source_sc, target_sc = [], [], [] 
                for i, ((im_source_val, label_source), (im_target_val, label_target)) in enumerate(iters):
                    s_b, t_b = im_source_val.shape[0], im_target_val.shape[0]
                    if s_b !=CFG.batch_size or t_b != CFG.batch_size:
                        continue
                    im_source_val = im_source.to(DEVICE)
                    im_target_val = im_target.to(DEVICE)
                ...
                print('Epoch [{}/{}], Step [{}/{}], d_loss_val: {:.4f},  D(source_val): {:.2f}, D(target_val): {:.2f}' 
                  .format(epoch, args.n_epoch, i+1, iters, s.mean(d_losses),  
                          s.mean(source_sc),  s.mean(target_sc)))
                wandb.log({"d_loss_val": s.mean(d_losses),
                    "source_score_val": s.mean(source_sc),
                    "target_score_val": s.mean(target_sc)
                    })
